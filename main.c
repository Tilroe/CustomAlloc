...
int main(void) {
    ...
    // When freeing, should consume the next block (FREE block that stretches to the end of memory) (alloc.c line 100)
    int items = 8;
    ...
    // So trying to allocate for 254 ints should fail (alloc.c line 77)
    int* should_fail = (int*)alloc_custom(sizeof(int) * 254);

    // 4. Working with metadata (or lack thereof) at the end of
    ...
    //          because 3 bytes is not enough for a 9 byte metadata (alloc.c line 82 call exits early (alloc.c line 57))
    //      4.2: When freeing the above block, we try to read the next metadata to see if we can combine with a free
    //          block. The metadata read fails because it would go beyond the limits of memory (alloc.c line 96 exits
    //          early (alloc.c line  45)). We then know this block is at the end of memory, so when freeing we give
